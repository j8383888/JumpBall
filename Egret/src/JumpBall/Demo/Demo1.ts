module JumpBall {
	export class Demo1 extends eui.Component {
		...
		public creatRectBody(x, y, w, h, mess: number = 1, angularVelocity = 1): p2.Body {
			...
			//添加方形刚体
			var boxShape: p2.Shape = new p2.Box({ width: w / factor, height: h / factor });
			var boxBody: p2.Body = new p2.Body({ mass: mess, position: [x / factor, y / factor], angularVelocity: angularVelocity });
			boxBody.addShape(boxShape);
			this.world.addBody(boxBody);
			display = this.createBox(w, h);
		...
		public creatCircleBody(x, y, r, mess = 0, angularVelocity = 1): p2.Body {
			...
			var display: egret.DisplayObject;
			//添加方形刚体
			var boxShape: p2.Shape = new p2.Circle({ radius: r / factor });
			var boxBody: p2.Body = new p2.Body({ mass: mess, position: [x / factor, y / factor], angularVelocity: angularVelocity });
			boxBody.addShape(boxShape);
			this.world.addBody(boxBody);
			display = this.createBall(r);
			boxBody.displays = [display];
			this.addChild(display);
			return boxBody;
		...
		private _init(): void {
			...
			function addOneBox(e: egret.TouchEvent): void {
				...
				if (Math.random() > 1) {
					...
					var boxShape: p2.Shape = new p2.Circle({ radius: 2 });
